...
#         du[i, :] .= Clump(du[i, :], u[i, :], p.clumps[i], t) + sum([Fs(u[i,:], u[j,:], p.springs[i, j]) for j = 1:length(p.clumps) if j != i])
#     end
# end
...
function Raft!(du, u, p, t)
    for i = 1:size(u, 1)
        x = u[i, 1]
        y = u[i, 2]

        α, τ, R, f = (0.005426657127666697, 0.018636881902249026, 0.7883113745656047, 2.1821273570707342)
        ...
        for j = 1:size(u, 1)
            if j != i
                ...
                du[i, 1] += 20*(0.4/norm(xyi - xyj) - 1)*(xyi[1] - xyj[1])
                du[i, 2] += 20*(0.4/norm(xyi - xyj) - 1)*(xyi[2] - xyj[2])

                # du[i, 1] += 20*(0.4/1 - 1)*(xyi[1] - xyj[1])
                # du[i, 2] += 20*(0.4/1 - 1)*(xyi[2] - xyj[2])

                # du[i, 1] += 0.01*(1/norm(xyi - xyj) - 1)*(xyi[1] - xyj[1])
                # du[i, 2] += 0.01*(1/norm(xyi - xyj) - 1)*(xyi[2] - xyj[2])
...
function get_prob(n, tmax)
    ...
    tspan_raft = (0.0, tmax)
    prob_raft = ODEProblem(Raft!, raft_parameters.xy0, tspan_raft)
