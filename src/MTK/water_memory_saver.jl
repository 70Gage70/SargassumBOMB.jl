...
function clump_t(n_traj_x, n_traj_y, t_short, t_long, t_span)
    ...
    α, τ, R, f = params.α, params.τ, params.R, params.f
    τ = 0.0
    params = ClumpParameters(ref_itp, α, τ, R, f)
...
function all_clumps(n_traj_x, n_traj_y, t_short, t_long, t_stop)
    ...
    matwrite("x0xT_BOM_water_$(ts)days.mat", Dict("x0" => x0_short, "y0" => y0_short, "xT" => xT_short, "yT" => yT_short))
    matwrite("x0xT_BOM_water_$(tl)days.mat", Dict("x0" => x0_long, "y0" => y0_long, "xT" => xT_long, "yT" => yT_long))
